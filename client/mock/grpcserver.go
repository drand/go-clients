...
func NewMockGRPCPublicServer(t *testing.T, l log.Logger, bind string, badSecondRound bool, sch *crypto.Scheme, clk clock.Clock) (Listener, Service) {
	...
	listener, err := NewGRPCListenerForPrivate(ctx, bind, server)
	if err != nil {
		panic(err)
	}
...
func NewMockServer(t *testing.T, badSecondRound bool, sch *crypto.Scheme, clk clock.Clock) Service {
	d := generateMockData(sch, clk)
	testValid(d)
