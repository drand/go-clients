...
var accessLogFlag = &cli.StringFlag{
	Name:    "access-log",
	Usage:   "file to log http-relay accesses to",
...
func Relay(c *cli.Context) error {
	...
	if hashFlagSet {
		return fmt.Errorf("--%s is deprecated on relay http-relay, please use %s instead", lib.HashFlag.Name, lib.HashListFlag.Name)
	}

	handler, err := dhttp.New(context.Background(), fmt.Sprintf("drand/%s (%s)",
		version, gitCommit))
	if err != nil {
	...
	// http-relay.Serve is marked as problematic because it does not
