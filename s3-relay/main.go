	...
	INNER:
		for {
			...
			case res, ok := <-ch:
				...
				l.Infow("", "relay_s3", "got randomness", "round", res.GetRound())
				go func(res client.Result) {
					...
					l.Infow("", "relay_s3", "uploaded randomness", "round", res.GetRound(), "location", url)
				}(res)
			case <-ctx.Done():
				return
			}
...
func uploadRandomness(ctx context.Context, upr *s3manager.Uploader, buc string, res client.Result) (string, error) {
	...
	r, err := upr.UploadWithContext(ctx, &s3manager.UploadInput{
		...
		Key:          aws.String(fmt.Sprintf("public/%v", res.GetRound())),
...
var syncCmd = &cli.Command{
	...
	Action: func(cctx *cli.Context) error {
		...
		for rnd := cctx.Uint64("begin"); rnd <= c.RoundAt(time.Now()); rnd++ {
			...
			lg.Infow("", "relay_s3_sync", "uploaded randomness", "round", r.GetRound(), "location", url)
		}
